...
if ( ! isset( $_COOKIE[ $cookie_name ] ) ) { ?>
    ...
    <div id="cookie" class="cookie js-cookie" role="dialog" aria-modal="true" aria-label="cookieconsent">
        <div class="cookie__wrapper">
            ...
            <form id="cookie-form" class="cookie__container js-cookie-form" method="POST" action="<?php echo admin_url( 'admin-post.php' ); ?>">
                ...
                <?php if ( $cookie_body ) { ?>
                    <div class="cookie__body">
                        <?php echo apply_filters( 'the_content', $cookie_body ); ?>
                    </div>
                <?php } ?>
                ...
                <div class="cookie__buttons">
                    ...
                    <button type="submit" 
                            name="accept" 
                            class="button cookie__button cookie__button--accept js-cookie-accept">
                                <?php if ( $cookie_accept_label ) { 
                                    echo $cookie_accept_label; 
                                } else { 
                                    _e( 'Accept', THEME_TEXT_DOMAIN ); 
                                } ?>
                    </button>
                    ...
                    <?php if ( $cookie_refuse_active ) { ?>
                        <button type="submit" 
                                name="refuse" 
                                class="button cookie__button cookie__button--refuse js-cookie-refuse">
                                    <?php if ( $cookie_refuse_label ) { 
                                        echo $cookie_refuse_label; 
                                    } else { 
                                        _e( 'Refuse', THEME_TEXT_DOMAIN ); 
                                    } ?>
                        </button>
                    <?php } ?>

                    <?php if ( $cookie_read_more_active ) { ?>
                        <a  href="<?php if ( $cookie_read_more_page ) { the_permalink( $cookie_read_more_page ); } else { echo '#'; } ?>" 
                            class="button cookie__button cookie__button--read-more js-cookie-read-more" 
                            target="_blank">
                            <?php if ( $cookie_read_more_label ) { 
                                echo $cookie_read_more_label; 
                            } else { 
                                _e( 'Cookie policy', THEME_TEXT_DOMAIN ); 
                            } ?>
                        </a>
                    <?php } ?>
...
<?php 
// Revoke cookie form
} else if ( $cookie_revoke_active && isset( $_COOKIE[ $cookie_name ] ) && $_COOKIE[ $cookie_name ] === 'false' ) { ?>

    <div id="cookie" class="cookie cookie--revoke js-cookie">
        <form id="cookie-form" class="js-cookie-form" method="POST" action="<?php echo admin_url( 'admin-post.php' ); ?>">
